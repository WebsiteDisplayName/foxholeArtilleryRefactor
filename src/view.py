...
with dpg.window(tag="Primary Window", label="main", pos=(200, 200)):
    with dpg.menu_bar(tag="mainMenuBar"):
        ...
        dpg.add_combo(tag="weaponDropdown", items=list(cH.WEAPONTYPEWINDFORCES.keys()), default_value=list(cH.WEAPONTYPEWINDFORCES.keys())[0], callback=cT.update_firing_solution, user_data=["global", 6], width=120)

        dpg.add_text(default_value="Wind Force")
        dpg.add_combo(tag="wind_forceDropdown", items=[1, 2, 3], default_value=1,
                      callback=cT.update_firing_solution, user_data=["global", 7], width=95)
        dpg.add_text(default_value="Wind Azi")
        dpg.add_input_int(tag="globalwind_azimuth",
                          default_value=0.0, step=0, step_fast=0, callback=cT.update_firing_solution, user_data=["global", 8], width=30)
        dpg.add_combo(tag="file_optionsDropdown", items=["Open FS", "Save FS"], default_value="Open FS",
                      callback=cT.file_options, width=100)
